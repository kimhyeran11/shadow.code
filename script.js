...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.asset-item[data-type="decoration"]').forEach(item => {
        item.addEventListener('click', () => {
            ...
            } else if (canvasImageSrc.includes('가로수.png')) { // 가로수 크기 조정
                initialHeight = 500;
                initialWidth = (700 / 1200) * initialHeight; // 가로수 이미지 비율에 맞춰 조정
            } else if (canvasImageSrc.includes('의자.png')) { // 의자 크기 조정
                initialHeight = 200;
                initialWidth = (500 / 300) * initialHeight; // 의자 이미지 비율에 맞춰 조정
            } else if (canvasImageSrc.includes('배.png')) { // 배 크기 조정
                initialHeight = 300;
                initialWidth = (800 / 400) * initialHeight; // 배 이미지 비율에 맞춰 조정
            }
    ...
    exportButton.addEventListener('click', () => {
        ...
        // ==========================================================
        // ⭐ 2. 캔버스 크기 변환 로직 (핵심 수정) ⭐
        // ==========================================================
        // Mobile 캔버스 (목표 크기) 기준값 설정
        // Mobile.html의 #canvas는 width: 80%를 가집니다. Index.html의 #canvas (width: 90%) 보다 작습니다.
        // Mobile.html의 캔버스 가로/세로 비율 (16:9)에 맞춘 임시 기준값 (예: 800px x 450px)
        const TARGET_CANVAS_WIDTH = 800;
        const TARGET_CANVAS_HEIGHT = 450;
    ...
    document.querySelectorAll('.scene').forEach(scene => {
        // 썸네일 업데이트 시 캔버스 크기 참조가 필요할 수 있으므로 setTimeout으로 지연
